...
subtitle: "Accident Prevention Analytics Consultant Group"
...
## 1. Monthly Overview
First, let's take a peek at monthly data with different severities. 
...
```{r}
ggp2 <- accidents %>%
  select(s_month, s_year ,Severity) %>%
  ggplot()+
  geom_bar(mapping = aes(x= s_month), fill= "brown") + 
  facet_wrap(~Severity) + 
  labs(x= "Months",
       y= "count of accidents",
       title= "Monthly view for accidents")
ggp2 +
  scale_y_continuous(labels = comma)
```
## 2. Day and night time impact on accidents
After we analyze the month, we use bar chart to show the count of accidents in day and night time, then we use scales to format numbers on y axis. 
Select method was used to limit the data size for the chart.
...
Ok, now we have day and night impact on accidents, what about the relationship between severity and its distance impacted between day and night time?
Will it be like that the higher severity the longer the distance affected would be? Also does day time and night time have different impact on them?

Let's use ggplot and geom point, and put Severity on the X axis and Distance on the Y axis. We set legend by Sunrise_Sunset.Also, utilized labs functions to give titles to axis and graph.
...
accidents %>%
  ...
  ggplot() +
  geom_point(mapping = aes(x= Severity, y= Distance))+
...
## 3. Does weather impact a lot?
Now we know that the distance impacted is not that big. The biggest impact on the distance is a severity 3 which had an impact of more than 400 miles. Now let's take a look into weather condition impact. 
The chart below shows the top 10 weather conditions when accidents happened.
We have used group by and summarize function to count accidents in each weather condition and selected top 10 weather conditions using the functions arrange descending,top_n. 
...
## 4. Severity Analysis in Day/Night: 
...
ggp2 + 
  scale_y_continuous(labels= comma)
```
So apparently it's more dangerous driving in the day although the visibility is better. So what are the most dangerous highways?

## 5. Top 10 most dangerous Highways?: 

In this analysis to found top 10 highway/Street which has maximum count of accidents. In this analysis, we used select function to select street, city and state. Used group by to count accidents by street. Additionally used summarize to count number of accidents and arrange descending to get maximum accidents, and top_n to get only top 10 results. After getting the appropriate data for the visualization, we plot the data using ggplot and grom bar graph to visualize top 10 highways. Utilized labs function to give title to axis and title to graph.
...
ggplot(data =  top_state)+
    geom_point(mapping = aes(State,count), colour= "blue")+
...
```{r}
# chandrark, add comments
ggp6 <- ggplot(data = accidents)+
  geom_bar(mapping = aes(x = Severity))+
  facet_wrap(~s_year,nrow = 4)+ 
  labs(x= "Severity in each year",
       y= "Number of Accidents",
       title = "Severity by years")
